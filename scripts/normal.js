...
function carregar_arquivos(arquivos){
  arquivos.forEach((arquivo, indice) => {
    if (arquivo && arquivo.type === 'image/svg+xml') {
...
const soltar_arquivos = document.getElementById('editar');

soltar_arquivos.addEventListener('dragover', (e) => {
  e.preventDefault();
  div_vizualizar_svg.style.borderColor = 'var(--cor-azul)';
  div_texto_svg.style.borderColor = 'var(--cor-azul)';
});

soltar_arquivos.addEventListener('dragleave', () => {
  div_vizualizar_svg.style.borderColor = 'var(--cor-borda)';
  div_texto_svg.style.borderColor = 'var(--cor-borda)';
});
